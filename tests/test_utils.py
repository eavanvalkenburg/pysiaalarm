...
def create_test_items(key, content):
    ...
    # )
    encrypter = AES.new(key.encode("utf8"), AES.MODE_CBC, Random.new().read(AES.block_size))
    ...
    return hexlify(encrypter.encrypt(unencrypted.encode("utf8"))).decode(encoding="UTF-8").upper()
